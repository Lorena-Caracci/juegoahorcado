...
var listaPalabras = ["perro", "gato","conejo","raton","jirafa","pez","murcielago","hipopotamo","lobo","elefante","gallina","cabra","vaca","toro","cerdo","oso","serpiente","puma","aguila","leon","avestruz","tigre","loro","mono","caballo","cebra","canguro","burro"];
...
function initGame() {
    ...
    let tecladof1 = "qwertyuiop";
    let tecladof2 = "asdfghjklñ";
    let tecladof3 = "zxcvbnm";

    let f1 = document.querySelector(".fila1");
    for (let i = 0; i < tecladof1.length; i++) {
        const tecla = document.createElement("button");
        tecla.classList = "tecla";
        tecla.textContent = tecladof1[i].toUpperCase();
        tecla.id = tecladof1[i].toString();
        tecla.addEventListener("click", function () { teclado(this, palabra); });
        f1.append(tecla);
    }

    let f2 = document.querySelector(".fila2");
    for (let i = 0; i < tecladof2.length; i++) {
        const tecla = document.createElement("button");
        tecla.classList = "tecla";
        tecla.textContent = tecladof2[i].toUpperCase();
        tecla.id = tecladof2[i].toString();
        tecla.addEventListener("click", function () { teclado(this, palabra); });
        f2.append(tecla);
    }

    let f3 = document.querySelector(".fila3");
    for (let i = 0; i < tecladof3.length; i++) {
        const tecla = document.createElement("button");
        tecla.classList = "tecla";
        tecla.textContent = tecladof3[i].toUpperCase();
        tecla.id = tecladof3[i].toString();
        tecla.addEventListener("click", function () { teclado(this, palabra); });
        f3.append(tecla);
    }
    return;
...
function gameOver() {
    if(document.querySelector(".state-nodisplay") != null){
        ...
        game.textContent = "AHORCADO!";/*palabra[i].toUpperCase()*/
...
const modo = document.querySelector('#modo'); // modo oscuro
modo.addEventListener('click', function () {
    ...
    if (window.modoOscuro == false) {
        ...
        document.body.style.background = "#211f01 url('modooscuro.jpg') no-repeat center top";
        ...
        r.style.setProperty('--color1', '#0a110f'); 
        r.style.setProperty('--color2', '#D8FFDB');
        r.style.setProperty('--color3', '#FF69F5');
        r.style.setProperty('--color4', '#10e9d3');
        r.style.setProperty('--color5', '#5f217c');
        window.modoOscuro = false
        this.src = "modooscuro.jpg"
        let intento_back = window.intento
        window.intento = 0
        for (let i = 0; i < intento_back; i++) {
            drawGame()
        ...
        document.body.style.background = "#FF69F5 url('modoclaro.jpg') no-repeat center top";
        document.body.style.backgroundSize = "100%";

        const logo = document.querySelector('#logo');
        logo.style.fill = "#D8FFDB"
        const exclam = document.querySelector('#exclamacion');
        exclam.style.fill = "#D8FFDB"
        const text_modo = document.querySelector('#text_modo');
